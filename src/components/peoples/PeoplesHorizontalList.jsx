...
const PeoplesHorizontalList = ({
  ...
  const [movieData, setMovieData] = useState(() => {
    return data?.data.filter((movie) => movie.profile_path);
  });
  ...
  useEffect(() => {
    ...
    if (inView) {
      ...
      const fetchInitialQuery = async () => {
        ...
        const filter = query.data.filter((movie) => movie.profile_path);
        setMovieData(filter);
  ...
  const maxIndex = useMemo(() => {
    ...
    let maxIndexGoesTo;
    if (movieData.length % numberOfProfile === 0) {
      maxIndexGoesTo = -(movieData.length / numberOfProfile - 1);
    } else {
      const divide = movieData.length / numberOfProfile;
